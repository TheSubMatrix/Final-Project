    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Fragment(VertexToFragment input) : SV_Target
            {
                ...
                float normalEdge = dot(normalSamples, input.ViewSpaceDir);
                float depthEdge = sign(depthSamples) * abs(max(depthSamples - 0.1, 0));
                return clamp(1 - (depthEdge + normalEdge) * -1, -2, 1) * SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, input.Texcoord);
